...
import { getOrdersBySupplier, updateOrderStatus } from '../../services/supabase'
              ...
              <div key={order.id} className="bg-white rounded-2xl shadow-sm overflow-hidden">
                ...
                >
                  <div className="flex items-start justify-between">
                    <div>
                      <p className="font-bold text-gray-900">{order.buyer?.company_name || 'Comprador'}</p>
                      <p className="text-xs text-gray-400 mt-0.5">{order.created_at ? formatDate(order.created_at) : ''}</p>
                      <p className="text-xs text-gray-500 mt-1">{order.items?.length || 0} itens</p>
                    </div>
                    ...
                    {order.buyer && (
                      <div className="mt-3 p-3 bg-gray-50 rounded-xl space-y-1">
                        ...
                        <p className="text-xs text-gray-500">
                          {order.buyer.address_street}, {order.buyer.address_number} - {order.buyer.address_neighborhood}
                        </p>
                        <p className="text-xs text-gray-500">
                          {order.buyer.address_city}/{order.buyer.address_state} - CEP: {order.buyer.address_zip}
                        </p>
                        <button
                          onClick={() => openWhatsApp(order.buyer!.contact_phone, encodeURIComponent(`Olá! Seu pedido no VerdeMar (${formatCurrency(order.total_value)}) está sendo processado.`))}
